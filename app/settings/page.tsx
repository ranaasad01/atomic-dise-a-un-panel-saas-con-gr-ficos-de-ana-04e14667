...
const ACCENT_COLORS: AccentColor[] = [
  { name: "Índigo", value: "#6366F1", class: "bg-sky-500" },
  { name: "Violeta", value: "#8B5CF6", class: "bg-sky-500" },
  { name: "Cian", value: "#06B6D4", class: "bg-sky-500" },
  { name: "Esmeralda", value: "#10B981", class: "bg-sky-500" },
  { name: "Rosa", value: "#EC4899", class: "bg-pink-500" },
  { name: "Ámbar", value: "#F59E0B", class: "bg-sky-500" },
];
...
function Toggle({
  ...
  return (
    <button
      ...
      className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 focus-visible:ring-offset-2 focus-visible:ring-offset-[#0F172A] ${
...
function PerfilTab() {
  ...
  const inputClass =
    "w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-sky-500/60 focus:border-sky-500/40 transition-all duration-200";
      ...
      <SectionCard>
        ...
        <div className="flex items-center gap-5">
          <div className="relative shrink-0">
            <div className="h-20 w-20 rounded-2xl bg-gradient-to-br from-sky-500/40 to-sky-600/40 border border-sky-500/30 flex items-center justify-center text-2xl font-bold text-sky-300 select-none">
              AR
            </div>
            <button
              type="button"
              className="absolute -bottom-1.5 -right-1.5 flex h-7 w-7 items-center justify-center rounded-full bg-sky-500 border-2 border-[#0F172A] text-white hover:bg-sky-400 transition-colors duration-200"
          ...
          <div>
            ...
            <button
              type="button"
              className="mt-2 text-xs font-medium text-sky-400 hover:text-sky-300 transition-colors duration-200"
            >
              Subir nueva imagen
            </button>
          </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/8">
            <div className="flex items-center gap-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-sky-500/15 border border-sky-500/25">
            ...
            <button
              type="button"
              className="text-xs font-medium text-sky-400 hover:text-sky-300 transition-colors duration-200 flex items-center gap-1"
            >
        ...
        <motion.button
          ...
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-300 ${
            saved
              ...
              : "bg-sky-500 hover:bg-sky-400 text-white shadow-[0_0_20px_rgba(99,102,241,0.35)]"
          }`}
        >
          {saved ? (
            <>
    ...
    <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="space-y-6">
      <SectionCard>
        <div className="flex items-center gap-3 mb-6">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-sky-500/15 border border-sky-500/25">
          ...
          {(["dark", "light", "system"] as const).map((t) => {
            ...
            return (
              ...
              >
                ...
                />
                <span className={`text-xs font-medium ${active ? "text-sky-300" : "text-slate-400"}`}>
                  {labels[t]}
                </span>
                {active && (
                  <span className="absolute top-2 right-2 flex h-4 w-4 items-center justify-center rounded-full bg-sky-500">
        ...
        <div className="space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs font-mono text-sky-400">{fontSize}px</span>
            </div>
            <input
              type="range"
              min={12}
...
function FacturacionTab() {
  ...
  return (
    <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="space-y-6">
      ...
      <motion.div
        variants={fadeInUp}
        className="relative overflow-hidden rounded-2xl border border-sky-500/30 bg-gradient-to-br from-sky-500/10 via-sky-500/5 to-transparent p-6"
      >
          ...
          <div className="flex items-center gap-1 p-1 rounded-xl bg-white/5 border border-white/10">
            {(["mensual", "anual"] as const).map((cycle) => (
              ...
              >
                {cycle}
                {cycle === "anual" && (
                  <span className="ml-1.5 text-[10px] font-semibold text-sky-400">-20%</span>
                )}
              </button>
            ))}
          </div>
            ...
            return (
              <motion.div
                ...
                className={`relative flex flex-col rounded-2xl border p-5 transition-all duration-300 ${
                  plan.highlight
                    ? "border-sky-500/50 bg-sky-500/10 shadow-[0_0_30px_rgba(99,102,241,0.15)]"
                    : "border-white/10 bg-white/5"
                }`}
              >
                {plan.current && (
                  <span className="absolute -top-2.5 left-1/2 -translate-x-1/2 text-[10px] font-bold uppercase tracking-widest bg-sky-500 text-white px-3 py-0.5 rounded-full">
                    Actual
                  </span>
                )}
                {plan.highlight && !plan.current && (
                  <span className="absolute -top-2.5 left-1/2 -translate-x-1/2 text-[10px] font-bold uppercase tracking-widest bg-sky-500 text-white px-3 py-0.5 rounded-full flex items-center gap-1">
                ...
                <button
                  ...
                  className={`w-full py-2 rounded-xl text-xs font-semibold transition-all duration-200 ${
                    plan.current
                      ...
                      : plan.highlight
                      ? "bg-sky-500 hover:bg-sky-400 text-white shadow-[0_0_16px_rgba(99,102,241,0.4)]"
  ...
  return (
    <main className="min-h-screen bg-[#0F172A] pt-8 pb-24">
      ...
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        <div className="absolute -top-40 left-1/2 -translate-x-1/2 h-[500px] w-[700px] rounded-full bg-sky-600/8 blur-[120px]" />
      </div>
            ...
            <nav className="flex lg:flex-col gap-1">
              {TABS.map((tab) => {
                ...
                return (
                  ...
                  >
                    <Icon className={`h-4 w-4 shrink-0 ${active ? "text-sky-400" : "text-slate-500"}`} />
                    {tab.label}
                  </motion.button>
                );
              })}
