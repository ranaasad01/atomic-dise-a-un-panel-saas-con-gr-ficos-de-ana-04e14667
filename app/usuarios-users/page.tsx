...
const planBadgeMap: Record<string, string> = {
  Enterprise: "bg-rose-500/15 text-rose-300 border-rose-500/25",
...
export default function UsersPage() {
  ...
  const SortIcon = ({ col }: { col: SortKey }) => {
    ...
    return sortDir === "asc" ? (
      <ArrowUp className="h-3 w-3 text-rose-400 ml-1" />
    ) : (
      <ArrowDown className="h-3 w-3 text-rose-400 ml-1" />
    );
  };
          ...
          >
            ...
            <motion.div variants={fadeInUp} className="flex items-center gap-2">
              ...
              <motion.button
                whileHover={{ scale: 1.03 }}
                whileTap={{ scale: 0.97 }}
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-rose-500 hover:bg-rose-400 text-sm font-semibold text-white shadow-[0_0_20px_rgba(99,102,241,0.3)] transition-all duration-200"
              >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 p-5 border-b border-white/5">
            <div className="relative flex-1 max-w-sm">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 rounded-xl border border-white/8 bg-white/5 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-rose-500/50 focus:bg-white/8 transition-all duration-200"
              />
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              <div className="relative">
                <select
                  ...
                  className="appearance-none pl-3 pr-8 py-2 rounded-xl border border-white/8 bg-white/5 text-sm text-slate-300 focus:outline-none focus:border-rose-500/50 transition-all duration-200 cursor-pointer"
                >
                    ...
                    <button
                      ...
                      className={`h-4 w-4 rounded border flex items-center justify-center transition-all duration-200 ${
                        selectedIds.length === filtered.length && filtered.length > 0
                          ...
                          : "border-white/20 bg-white/5 hover:border-rose-500/50"
                      }`}
                    >
                      {selectedIds.length === filtered.length && filtered.length > 0 && (
