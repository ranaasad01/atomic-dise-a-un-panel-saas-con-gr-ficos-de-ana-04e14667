...
function SaveBanner({ visible }: { visible: boolean }) {
  return (
    <motion.div
      ...
      className="fixed bottom-6 right-6 z-50 flex items-center gap-3 rounded-xl border border-indigo-500/30 bg-indigo-500/20 px-5 py-3 shadow-[0_8px_32px_rgba(99,102,241,0.25)] backdrop-blur-xl"
    >
      <Check className="h-4 w-4 text-indigo-300" />
      <span className="text-sm font-medium text-indigo-200">Cambios guardados correctamente</span>
    </motion.div>
  );
}
...
function InputField({
  ...
  return (
    <input
      ...
      disabled={disabled}
      className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-200 placeholder-slate-500 outline-none transition-all duration-200 focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
    />
  );
}
...
function SelectField({
  ...
  return (
    <select
      value={value}
      onChange={(e) => onChange(e.target.value)}
      className="w-full rounded-lg border border-white/10 bg-[#1E293B] px-3 py-2 text-sm text-slate-200 outline-none transition-all duration-200 focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/20"
    >
      {options.map((o) => (
    ...
    <motion.div variants={staggerContainer} initial="hidden" animate="visible">
      ...
      <motion.div variants={fadeInUp} className="mb-6 flex items-center gap-5 rounded-xl border border-white/8 bg-white/3 p-5">
        <div className="relative">
          <div className="h-16 w-16 rounded-full bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-xl font-bold text-white shadow-[0_0_0_3px_rgba(99,102,241,0.3)]">
            AR
          </div>
      ...
      <motion.div variants={fadeInUp} className="rounded-xl border border-white/8 bg-white/3 px-5 divide-y divide-white/5">
        ...
        <FieldGroup label="Biografía" hint="Breve descripción visible en tu perfil público.">
          <textarea
            ...
            rows={3}
            className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-200 placeholder-slate-500 outline-none transition-all duration-200 focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/20 resize-none"
          />
        </FieldGroup>
      ...
      <motion.div variants={fadeInUp} className="mt-5 flex justify-end">
        <motion.button
          ...
          whileTap={{ scale: 0.97 }}
          onClick={onSave}
          className="flex items-center gap-2 rounded-lg bg-indigo-500 px-5 py-2.5 text-sm font-semibold text-white shadow-[0_0_20px_rgba(99,102,241,0.35)] hover:bg-indigo-400 transition-all duration-200"
        >
          ...
          <div className="grid grid-cols-3 gap-3">
            {themes.map((t) => (
              ...
              >
                ...
                {theme === t.id && <Check className="h-3 w-3 text-indigo-400" />}
              </motion.button>
            ))}
          </div>
        </div>
...
function IntegracionesTab() {
  ...
  return (
    <motion.div variants={staggerContainer} initial="hidden" animate="visible">
      ...
      <motion.div variants={fadeInUp} className="mb-5 rounded-xl border border-indigo-500/20 bg-indigo-500/8 px-5 py-4 flex items-start gap-3">
  ...
  return (
    <div className="min-h-screen bg-[#0F172A]">
      ...
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        <div className="absolute top-0 left-1/4 h-96 w-96 rounded-full bg-indigo-500/6 blur-3xl" />
                ...
                return (
                  <motion.button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 text-sm font-medium transition-all duration-200 ${
                      ...
                      active
                        ? "bg-indigo-500/15 text-indigo-300 border-l-2 border-l-indigo-500"
