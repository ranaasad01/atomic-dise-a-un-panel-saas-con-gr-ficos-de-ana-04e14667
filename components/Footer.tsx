        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <motion.div variants={fadeInUp} className="space-y-3">
            <Link href="/" className="flex items-center gap-2 group w-fit">
              <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-sky-500/20 border border-sky-500/30 group-hover:bg-sky-500/30 transition-all duration-300">
              ...
              {navLinks.map((link) => (
                <li key={link.href}>
                  {renderLink(
                    ...
                    link.type,
                    link.label,
                    "text-sm text-slate-400 hover:text-sky-300 transition-colors duration-200"
                  )}
                </li>
              ))}
            </ul>
